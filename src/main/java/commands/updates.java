...
public class updates extends Command {
    ...
    protected void execute(CommandEvent event) {
        ...
        EB.addField("mudanças:","-Conserto de bugs\n-nerf no comando ``apostar``\n  >Reduzido premio da pizza de ``3x`` para ``2x``\n  >reduzido premio da cenoura de ``2x`` para ``o valor apostado mais metade do valor apostado``\n-Adicionado o comando ``update``",false);
        EB.addField("versão 2.2.4","correção de mais bugs e uma pequena falha de digitação",false);
